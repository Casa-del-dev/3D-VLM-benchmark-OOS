...
def _heading_deg_from_row(row: dict[str, Any]) -> float:
    """
    Compute the camera heading (yaw) in degrees from the T_world_device rotation matrix.
        The heading is defined as the angle between the camera's forward vector and the world forward direction,
        projected onto the horizontal plane. Positive angles indicate leftward rotation, negative angles indicate rightward rotation.
    """
