...
def _bucket_angle(abs_deg: float, no_motion_thresh_deg: float) -> str:
    if abs_deg < no_motion_thresh_deg:
        return "no significant rotation"
    if abs_deg < 15.0:
        return "about 10 degrees"
    return "about 20 degrees"


def determine_camera_rotation_answer(
    video_id: str,
    start_time_sec: float,
    end_time_sec: float,
    annotations_root: Path,
    no_motion_thresh_deg: float = 8.0,
) -> CameraRotationAnswer:
    ...
    angle_bucket = _bucket_angle(abs_deg, no_motion_thresh_deg)

    choices = [
        "left, about 10 degrees",
        "left, about 20 degrees",
        "right, about 10 degrees",
        "right, about 20 degrees",
        "no significant rotation",
    ]

    if angle_bucket == "no significant rotation":
        direction = "no significant rotation"
        correct_idx = 4
    elif signed_deg > 0:
        direction = "left"
        correct_idx = 0 if angle_bucket == "about 10 degrees" else 1
    else:
        direction = "right"
        correct_idx = 2 if angle_bucket == "about 10 degrees" else 3

    return CameraRotationAnswer(
        choices=choices,
...
def is_rotation_sample_stable(
    ...
    boundaries = [no_motion_thresh_deg, 15.0, 25.0]
    for b in boundaries:
        if abs(abs_deg - b) < margin_deg:
            return False
    return True