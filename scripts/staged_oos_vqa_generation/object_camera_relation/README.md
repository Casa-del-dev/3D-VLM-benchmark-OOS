...
This module generates **Out-of-Sight (OOS) object-camera relation staged VQA ** from video + annotations using a 3-stage pipeline:

1. **Merge in-view tracks & visibility tracks**
2. **Generate staged VQA questions**
3. **Visualize generated questions (optional)**

---

## Step 1 — Generate Visibility Tracks
...
---
# Step 2 — Merge In-view tracks & Visibility Tracks
## Run
```bash
VIDEO_ID=P04-20240413-142619

python scripts/.../prepare_tracks_for_generation_via_merging.py \
  --video_id $VIDEO_ID \
  ...
  --output_json scripts/.../outputs/$VIDEO_ID/merged_visibility_track.json
```
Output:
```
...
## Status definitation logic when merging
...
## Run
```bash
python staged_oos_question_generator.py --config staged_oos_question_generator_config.yaml
```

## Key Design

### Step 1 — Current Visibility
...
### Step 3 — Spatial Reasoning (Fixture)
...
---
### Step 5 — Camera-Relative Direction
...
---
### Step 4 — Camera-Relative Direction
(Step 4 camera pose question currently removed)
...
```bash
python scripts/staged_oos_vqa_generation/object_camera_relation/visualize_staged_oos_vqa.py \
  --questions outputs/generated_vqa/P04-20240413-142619_vqa.json \
  --video data/HD-EPIC/Videos/P04/P04-20240413-142619.mp4 \
  --output_dir scripts/staged_oos_vqa_generation/object_camera_relation/outputs/visualizations
```
