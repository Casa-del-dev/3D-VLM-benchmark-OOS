...
#     s5_meta = (step5 or {}).get("answer_metadata", {}) or {}
...
#         f"  metadata is_visible={s1_meta.get('is_visible')} "
...
#         "",
#         "Step 4: object-object relation",
#         f"  chosen label: {idx_to_choice((step4 or {}).get('choices', []), (step4 or {}).get('correct_idx'))}",
#         f"  anchor object: {s4_meta.get('object_y_name', 'N/A')} ({s4_meta.get('object_y_assoc_id', 'N/A')})",
#         f"  anchor pixel: {s4_meta.get('object_y_pixel', 'N/A')}",
#         f"  relative vector: {s4_meta.get('vector_object_x_relative_to_object_y', 'N/A')}",
#         "",
#         "Step 5: object-object distance",
#         f"  chosen label: {idx_to_choice((step5 or {}).get('choices', []), (step5 or {}).get('correct_idx'))}",
#         f"  distance_m: {s5_meta.get('distance_m', 'N/A')}",
#         f"  distance_bucket: {s5_meta.get('distance_bucket', 'N/A')}",
#     ]
#     return wrap_lines(lines)


# def draw_step4_relation_overlay(ax, label: Optional[str]) -> None:
#     if not label:
#         return
...
#     step4_meta = (step4 or {}).get("answer_metadata", {}) or {}
#     step5_meta = (step5 or {}).get("answer_metadata", {}) or {}
...
#         axes[0].set_title(f"Last stable visible frame\n t={last_visible_time}s | frame={last_idx}")
...
#         axes[0].set_title("Last stable visible frame")
#     axes[0].axis("off")

#     axes[1].imshow(query_frame)
#     axes[1].set_title(f"Query frame\n t={query_time}s | frame={query_idx} | fps≈{fps_used:.3f}")

#     # target projection (if any)
#     draw_marker(axes[1], step1_meta.get("projected_pixel"), qw, qh, "deepskyblue", "target")

#     # anchor object (NEW)
#     draw_marker(axes[1], step4_meta.get("object_y_pixel"), qw, qh, "lime", "anchor")

#     # step 4 relation overlay (quadrant)
#     draw_step4_relation_overlay(
#         axes[1],
#         idx_to_choice((step4 or {}).get("choices", []), (step4 or {}).get("correct_idx"))
#     )

#     # 👉 ADD THIS BLOCK HERE
#     distance_label = idx_to_choice(
#         (step5 or {}).get("choices", []),
#         (step5 or {}).get("correct_idx")
#     )

#     axes[1].text(
#         0.02, 0.98,
#         f"Step 5 distance: {distance_label}",
...
#         description="Visualize staged OOS questions for the current VQA format."
#     )
...
# python scripts/staged_oos_vqa_generation/object_object_relation/visualize_staged_oos_vqa.py \
#   --questions scripts/staged_oos_vqa_generation/object_object_relation/outputs/generated_vqa/P04-20240413-142619_vqa.json \
#   --video data/HD-EPIC/Videos/P04/P04-20240413-142619.mp4 \
#   --output_dir scripts/staged_oos_vqa_generation/object_object_relation/outputs/visualizations 
...
def build_text_panel(trajectory_id: str, traj: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "Step 1: visibility",
        f"  expected answer: {expected_visibility_label(step1)}",
        f"  metadata is_visible={s1_meta.get('is_visible')} is_stably_visible={s1_meta.get('is_stably_visible')} status={s1_meta.get('status')}",
        f"  projected_pixel={s1_meta.get('projected_pixel')}",
        "",
        "Step 2: last visible",
        f"  sampled_last_visible_time_sec: {s2_meta.get('sampled_last_visible_time_sec', 'N/A')}",
        f"  projected_pixel: {s2_meta.get('projected_pixel')}",
        f"  camera_coordinates: {s2_meta.get('camera_coordinates', 'N/A')}",
        f"  status: {s2_meta.get('status', 'N/A')}",
        f"  fixture: {s2_meta.get('fixture', 'N/A')}",
        "",
        "Step 3: closest fixture",
        ...
        f"  choices: {(step3 or {}).get('choices', [])}",
        "",
        "=== Parallel branches unlocked after Step 3 ===",
        "Branch 4a: object-camera relative position",
        f"  depends_on_steps: {(step4a or {}).get('depends_on_steps', 'N/A')}",
        ...
        f"  debug: {s4a_meta.get('debug', 'N/A')}",
        "",
        "Branch 4b: object-object relation",
        f"  depends_on_steps: {(step4b or {}).get('depends_on_steps', 'N/A')}",
        ...
        f"  anchor pixel: {s4b_meta.get('object_y_pixel', 'N/A')}",
        f"  relative vector: {s4b_meta.get('vector_object_x_relative_to_object_y', 'N/A')}",
        "",
        "Branch 4c: object-object distance",
        f"  depends_on_steps: {(step4c or {}).get('depends_on_steps', 'N/A')}",
...
def visualize_one(
    ...
    step1_meta = (step1 or {}).get("answer_metadata", {}) or {}
    step2_meta = (step2 or {}).get("answer_metadata", {}) or {}
    step4b_meta = (step4b or {}).get("answer_metadata", {}) or {}
    ...
    ax_last = fig.add_subplot(gs[0, 0])
    ax_query = fig.add_subplot(gs[1, 0])
    ax_text = fig.add_subplot(gs[:, 1])

    if last_frame is not None:
        ax_last.imshow(last_frame)
        ax_last.set_title(f"Incremental trunk: Step 2 last stable visible frame\n t={last_visible_time}s | frame={last_idx}")
        draw_marker(ax_last, step2_meta.get("projected_pixel"), lw, lh, "red", "last visible")
    else:
        ax_last.text(0.5, 0.5, "No last-visible frame available", ha="center", va="center")
        ax_last.set_title("Incremental trunk: Step 2 last stable visible frame")
    ax_last.axis("off")

    ax_query.imshow(query_frame)
    ax_query.set_title(f"Query frame with post-Step-3 branch overlays\n t={query_time}s | frame={query_idx} | fps≈{fps_used:.3f}")

    draw_marker(ax_query, step1_meta.get("projected_pixel"), qw, qh, "deepskyblue", "target", 0.04)
    draw_marker(ax_query, step4b_meta.get("object_y_pixel"), qw, qh, "lime", "anchor", 0.10)

    cam_label = idx_to_choice((step4a or {}).get("choices", []), (step4a or {}).get("correct_idx"))
    rel_label = idx_to_choice((step4b or {}).get("choices", []), (step4b or {}).get("correct_idx"))
    distance_label = idx_to_choice((step4c or {}).get("choices", []), (step4c or {}).get("correct_idx"))

    draw_quadrant_overlay(ax_query, cam_label, "Branch 4a", box_anchor=(0.05, 0.72))
    draw_quadrant_overlay(ax_query, rel_label, "Branch 4b", box_anchor=(0.73, 0.72))

    ax_query.text(
        0.02, 0.98,
        f"Branch 4c distance: {distance_label}",
    ...
    fig.suptitle(
        f"OOS question check: {trajectory_id}\nIncremental trunk (Steps 1-3) + parallel branches after Step 3",
        fontsize=14,
    )
    fig.tight_layout()
